...
export default function PhonecasePodTable({ title, subheader, tableLabels, tableData, ...other }) {
  return (
    <Card {...other}>
      ...
      <TableContainer sx={{ overflow: 'hidden', height: 460 }}>
        <Scrollbar>
          <Table sx={{ minWidth: 960 }}>
            ...
            <TableBody>
              {tableData.map((row) => (
                <PhonecasePodTableRow key={row.id} row={row} />
              ))}
            </TableBody>
          </Table>
        </Scrollbar>
...
function PhonecasePodTableRow({ row }) {
  ...
  return (
    <>
      <TableRow>
        ...
        <TableCell style={{ maxWidth: 100 }}>SPX Express</TableCell>
      </TableRow>
      <PhonecasePodSubRow row={row} />
    </>
  );
}

PhonecasePodTableRow.propTypes = {
  row: PropTypes.object,
};

function PhonecasePodSubRow({ row }) {
