...
export default function CarpetPodTable({ title, subheader, tableLabels, tableData, ...other }) {
  return (
    <Card {...other}>
      ...
      <TableContainer sx={{ overflow: 'hidden', height: 460 }}>
        <Scrollbar>
          <Table sx={{ minWidth: 960 }}>
            ...
            <TableBody>
              {tableData.map((row) => (
                <CarpetPodTableRow key={row.id} row={row} />
              ))}
            </TableBody>
          </Table>
        </Scrollbar>
...
function CarpetPodTableRow({ row }) {
  ...
  return (
    <>
      ...
      <CarpetPodSubRow row={row} />
    </>
  );
}

CarpetPodTableRow.propTypes = {
  row: PropTypes.object,
};

function CarpetPodSubRow({ row }) {
