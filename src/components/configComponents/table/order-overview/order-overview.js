...
export default function OrdersOverview({ title, subheader, tableLabels, tableData, ...other }) {
  return (
    <Card {...other}>
      ...
      <TableContainer sx={{ overflow: 'hidden', height: 460 }}>
        <Scrollbar>
          <Table sx={{ minWidth: 960 }}>
            ...
            <TableBody>
              {tableData.map((row) => (
                <OrdersOverviewRow key={row.id} row={row} />
              ))}
            </TableBody>
          </Table>
        </Scrollbar>
...
function OrdersOverviewRow({ row }) {
